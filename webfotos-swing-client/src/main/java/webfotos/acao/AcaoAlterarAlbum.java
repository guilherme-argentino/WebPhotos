...
public class AcaoAlterarAlbum extends AbstractAction {
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if (ev.getSource() == btNovo
				&& btNovo.getActionCommand() == PainelWebFotos.ACAO_NOVO) {
		...
		else if (btAlterar.getActionCommand() == PainelWebFotos.ACAO_CANCELAR
				&& ev.getSource() == btAlterar) {
		...
		else if ((btAlterar.getActionCommand() == PainelWebFotos.ACAO_ALTERAR && ev
				.getSource() == btAlterar)
				|| (btNovo.getActionCommand() == PainelWebFotos.ACAO_FINALIZAR && ev
						.getSource() == btNovo)) {
	...
	public void executaAlteracoes() {
		...
		// //////////////////////////////////////////////////////////////////////
		String caminhoAlbum = Util.getFolder("albunsRoot").getPath()
		...
		if (!diretorioAlbum.isDirectory()) {
			...
			if (!diretorioAlbum.mkdir()) {
				Util
						.log("[AcaoAlterarAlbum.executaAlteracoes.7]/ERRO: diretorio "
								+ caminhoAlbum
								+ " não pode ser criado. abortando");
				return;
			}
		}
		...
		for (int i = 0; i < fotos.length; i++) {
			...
			if (f.getCaminhoArquivo().length() > 0) {
				try {
					FileChannel canalOrigem = new FileInputStream(f
							.getCaminhoArquivo()).getChannel();
					FileChannel canalDestino = new FileOutputStream(
							caminhoAlbum + File.separator + f.getFotoID()
									+ ".jpg").getChannel();
					canalDestino.transferFrom(canalOrigem, 0, canalOrigem
							.size());
					canalOrigem = null;
					canalDestino = null;
				} catch (Exception e) {
					Util.log("[AcaoAlterarAlbum.executaAlteracoes.8]/ERRO: "
							+ e);
					sucesso = false;
				}
			}
	...
	private void prepareThumbsAndFTP(Foto[] fotos, int albumID,
		...
		for (int i = 0; i < fotos.length; i++) {
			...
			if (f.getCaminhoArquivo().length() > 0) {
				caminhoArquivo = caminhoAlbum + File.separator + f.getFotoID()
						+ ".jpg";
	...
	private void fireChangesToGUI(Foto[] fotos) {
		...
		((javax.swing.table.AbstractTableModel) tbAlbuns.getModel())
				.fireTableDataChanged();
		((javax.swing.table.AbstractTableModel) tbFotos.getModel())
				.fireTableDataChanged();
	...
	private void prepareExtraFiles(Album album, String caminhoAlbum) {
		...
		try {
			FileWriter out = new FileWriter(caminhoAlbum + File.separator
					+ album.getAlbumID() + ".js");
			out.write(album.toJavaScript());
			out.flush();
			out.close();

			out = new FileWriter(caminhoAlbum + File.separator
					+ album.getAlbumID() + ".xml");
	...
	public boolean recordFotoData(Foto[] fotos, int ultimoFotoID, int albumID) {
		...
		for (int i = 0; i < fotos.length; i++) {
			...
			if (nomeArquivo.length() > 0) {
				if (ultimoFotoID < 0) {
					try {
						...
						Util
								.log("[AcaoAlterarAlbum.recordFotoData]/ERRO: "
										+ e);
			...
			try {
				...
				sql = "SELECT fotoID,albumID,creditoID,legenda,altura,largura FROM fotos WHERE fotoID="
						+ fotoID;
	...
	public int recordAlbumData(Album album, int albumID) {
		...
		if (albumID == 0) {
			...
			try {
				...
				rowSet.moveToInsertRow();
				rowSet.updateInt("albumID", albumID);
				rowSet.updateDate("DtInsercao", new java.sql.Date((new Date())
						.getTime()));
				rowSet.insertRow();
		...
		// atualiza o banco de dados
		try {
			sql = "SELECT usuarioID,categoriaID,NmAlbum,Descricao,DtInsercao,albumID FROM albuns WHERE albumID="
					+ albumID;
